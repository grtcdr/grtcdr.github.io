#+title: Implementing per-project package management for ox-publish projects
#+date: <2023-02-07 Tue>
#+filetags: :emacs:

Emacs is an insanely liberating hacking environment, it is perpetually
sucking me in through its intricate systems, many of which help me
organize and automate different areas of my life.

A key part of the Emacs ecosystem are the Org Mode libraries which
cover many things from [[info:org#Exporting][exporting documents]] and helping you [[info:org#Tracking your habits][keep track
of habits]] to an [[info:org#Citations][intuitive citation system]] that can be combined with
other integral tools, like the magnificent [[https://github.com/andras-simonyi/citeproc-el][citeproc]].
...
achieving a task and how others are doing it.

I'm currently leveraging =ox-publish=, an immensely extensible and
highly documented facility of the Org Mode library, to build and
architecture this website, a website whose first commit happened
around seven months ago. I've added lots of tiny features since then:
[[https://emacs.ch/@grtcdr/109673543873822484][preference-based colorschemes]], [[https://emacs.ch/@grtcdr/109818123826436913][syntax highlighting]], [[https://emacs.ch/@grtcdr/109711989196426299][citations]], and a
[[https://git.sr.ht/~grtcdr/dotfiles/tree/main/item/emacs/.config/emacs/modules/ab-blog.el][tiny module]] that helps me navigate my blog post directory a lot more
efficiently, to name a few.
...
language. Emacs 29, which has not yet been released, thankfully added
a new =--init-directory= flag which we can use to specify a custom
=user-emacs-directory=, meaning the packages we install from say, a
publishing script, will not interfere with the user's general
environment.

To give you an example, we can consider citeproc a dependency of this
project, the way we'd go about handling this dependency follows this general ruleset:
...
#+begin_example
$ tree -L 2 -I 'archives|gnupg' .emacs.d
...
;; Examples:
;;   (op-package--vc-repo :github "grtcdr/liaison")
;;   (op-package--vc-repo :sourcehut "protesilaos/denote")
#+end_src

Although SourceHut prefixes usernames with a tilde, we managed to
offload that to the =op-package--vc-repo= function, our interactions
with it can therefore remain consistent.

Now, =op-package-install= should remain an entrypoint for installing
any kinds of packages, archive or not. It needs to undergo a tiny adjustment for that to be possible.
...
I'm actively using this setup to try and deliver a great reading
