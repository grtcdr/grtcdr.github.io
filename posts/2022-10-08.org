...
#+begin_src dot :file ../images/posts/org-publish-process.svg :exports results
digraph G {
    publish [
...
#+ATTR_HTML: :alt The publishing process from an Org file to an HTML document :height 300px :style padding:3em;
#+CAPTION: Overview of the publishing process of an Org document
#+RESULTS:
[[file:../images/posts/org-publish-process.svg]]
...
The objective is to redefine =org-html-format-spec=, and [[https://github.com/grtcdr/grtcdr.tn/blob/main/composite.el][composite.el]]
will be taking charge, as it controls the structure of the website by
providing its specification at the time of transpilation.
...
(defvar forges
  ...
  "Property list of mapping git forges to their respective domain.")
#+end_src
...
#+begin_src emacs-lisp :eval no
;; %w format string: expands to a link to the source code
;; of a given resource (blog post) hosted on GitHub.
(?w . (format
       "<a href=%s>source</a>"
       (concat
...
;; %x format string: expands to a link to the list of revisions
;; of a given resource (blog post) hosted on GitHub.
(?x . (format
       "<a href=%s>history</a>"
       (concat
...
;; %y format string: expands to a link to the list of revisions
;; of a given resource (documentation) hosted on SourceHut.
(?y . (format
       "<a href=%s>source</a>"
       (concat
	(build-forge-prefix-url :sourcehut "grtcdr/dotfiles" 'tree)
...
;; %z format string: expands to a link to the list of revisions
;; of a given resource (documentation) hosted on SourceHut.
(?z . (format
       "<a href=%s>history</a>"
       (concat
