...
+ [[https://www.vim.org/][Vim]]-like keybindings.
...
*** Key definitions
Let's define some modifiers and directional keys.
...
and as such, a modifier that is within your reach suddenly becomes a
necessity. I have chosen to map *caps lock to escape* to address
this. You can map all sorts of keys together with the use of
~xkb_options~. For more information, see xkeyboard-config(1).
...
*** Volume control
Control system volume with ~pactl~.
...
*** Configuration
Let's put that palette to good use, eh.
...
Let's configure some of Sway's looks, like the
color of the background and the typeface (which I only see in a window's titlebar).
...
The last thing I'dd do is instruct Sway to behave in a manner that
...
[fn:1] Display/keyboard backlight controller, I only make use of the display functionality however.
[fn:2] Lets us control system audio, among other things.
[fn:3] Lets us control currently playing media, through MPRIS.
