...
mechanism of the WLAN; we might consider spinning up a firewall such
as pfSense to protect the network from outsiders.

Also, traffic from inside the network is significantly slower than
what a typical cloud infrastructure might require.

We're also committing a grave mistake in terms of network
architecture, there's a single network serving all of the machines
(physical and virtual) which turns our little solution into a
...
2. This article explores an alternative way of creating a network
   infrastructure for a very specific academic project. It is in no
   way a definitive solution, nor is it a great one -- just one that
   works at no extra cost to me.
