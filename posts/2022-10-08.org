#+TITLE: Tracking blog post changes by leveraging version control systems
#+DATE: <2022-10-08 Sat>
#+SETUPFILE: ../setup.org
...
publicly available, it's not quite visible. I intend to fix that, and
this post is going to detail that experience.

We're going to build on top of =ox-html=, and we're going to make sure
that every blog post links back to its source code, this way I don't
have to write up a changelog for my posts, we'll be indirectly
leveraging Git's primary superpower, *source control*.
...
which are translated respectively by =ox-latex= into the author's name
and email.
...
Fortunately for us, this applies to =ox-html=, and the macro in
